...
for metal_id in Z:
    ...
    output_file = ostar_dir + 'ostar2002_' + metal_id + '_LW_flux.dat'
    ...
    for T in Teff:
        for gval in g:
            ...
            if ( gval == 3.0  and T >= 32500) or\
               ...
               LW_flux = 0.0 # flag that model is not on grid                
            else:
                ostar_data = np.genfromtxt(ostar_file, usecols = (0, i + 1) )

                # compute FUV flux from stars
                LW_flux = FUV.integrate_SED(ostar_data[:,0], ostar_data[:,1],
                                            nu_min = ((11.2 *u.eV)/const.h).to(u.Hz).value,
                                            nu_max = ((13.6 *u.eV)/const.h).to(u.Hz).value)
...
for metal_id in Z:
    ...
    LW_file        = ostar_dir + 'ostar2002_' + metal_id + '_LW_flux.dat'
    data_array[:,i] = np.genfromtxt(LW_file, usecols=2)

    i = i + 1



np.savetxt(ostar_dir + 'ostar2002_LW_flux_all_models.dat', data_array,
